...
> **Note:** This PCAP is a Wireshark sample capture (`http_gzip.cap`) used for training purposes. The 2004 timestamp is expected.
...
| user_agent | Mozilla/5.0 (X11; U; Linux ppc; rv:1.7.3) Gecko/20041004 Firefox/0.10.1 |
| host | 56c0aaf26f |
...
Given the PCAP size and the observed evidence, this appears to be normal HTTP traffic, as there is only a single request with a 200 OK response. The goal of the case is to document the triage flow: time-window scoping, pivots between Zeek (HTTP/Conn/Files), Snort review, and IOC extraction with reproducible evidence.
...
- Proper time-window adjustment is critical when importing old PCAPs; otherwise, Kibana returns empty results.
- A single `policy-violation` alert without additional suspicious indicators does not escalate severity by itself—context matters.
- Documenting the triage methodology ensures reproducibility and helps build consistent analyst workflows.
